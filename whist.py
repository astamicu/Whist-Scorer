...
# added number of player when enter the name (eg: enter player 1/2/3/x name)
# added number of total rounds when starting a new round (round x/total
#   number of round
...
def howManyPlayers():
    ...
    while len(playerList) != number_of_players:
        name = input('Enter player ' + str(len(playerList) + 1) + ' name : ')
...
def scoring():
    ...
    for i, (player, score, final) in enumerate(
            zip(playerList, scoreList, finalScores)):
